...
| `wgs_cohort_variants_bcftools_filtered_genotype_table.tsv` | Genotype table including genotype, depth, allele depth, and PL values for each sample at each site |
| `wgs_cohort_variants_bcftools_filtered_site_table.tsv` | Variant table including site-level quality, depth, allele depth, MQ, and filter for each site |
...
| `${sampleID}/bam/` | Folder containing BAM files saved with optional parameter `keep_intermediate` |
